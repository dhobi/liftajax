...
import code.util.{MyLiftRules, MyScriptRenderer}
...
class Boot {
  def boot {
    ...
    //Use the already available ajax stuff
    LiftRules.ajaxPostTimeout = 5000
    LiftRules.ajaxRetryCount = Full(0)
    LiftRules.ajaxDefaultFailure = Full(() => JsCmds.Alert(S ? "An error happened"))

    //Extend the ajax stuff
